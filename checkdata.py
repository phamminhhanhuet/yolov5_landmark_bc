...
def check_data():
    annotateds = [os.path.basename(f_path) for f_path in glob.glob('/home/hanhpm2/Downloads/BarcodeDatasets/balance_output_copy/images/train/*.jpg')]
    images = [os.path.basename(f_path) for f_path in glob.glob('/home/hanhpm2/Downloads/BarcodeDatasets/balance_output_copy/images/test/*.jpg')]
...
def show_results(img, xywh, landmarks):
    h, w, c = img.shape
    tl = 10 or round(0.02 * (h + w) / 2) + 1  # line/font thickness
    # tl = 5
    x1 = int(xywh[0] * w - 0.5 * xywh[2] * w)
    y1 = int(xywh[1] * h - 0.5 * xywh[3] * h)
    x2 = int(xywh[0] * w + 0.5 * xywh[2] * w)
    y2 = int(xywh[1] * h + 0.5 * xywh[3] * h)
    cv2.rectangle(img, (x1, y1), (x2, y2), (102, 11, 102), thickness=tl, lineType=cv2.LINE_AA)

    clors = [(255, 0, 0), (0, 255, 0), (0, 0, 255), (111, 124, 0)]
    print('Number of landmark: ', len(landmarks))
    for i in range(4):
...
def vis_data():
    image_dir = '/home/hanhpm2/Downloads/BarcodeDatasets/balance_output_copy/images/val/'
    label_dir = '/home/hanhpm2/Downloads/BarcodeDatasets/balance_output_copy/labels/val/'
    save_dir = '/home/hanhpm2/Downloads/BarcodeDatasets/visual_result/'
